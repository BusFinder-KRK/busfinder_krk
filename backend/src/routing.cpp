...
void Routing::load_walking_csv() {
  ...
  if (!file.is_open()) {
    throw std::ios_base::failure("Couldn't open the csv.");
  }
  std::string line;
  std::stringstream ss;
...
std::vector<Routing::NodeTransport> Routing::transport_between_stops(
  ...
  for (const auto &option : possible_trips) {
    if (option.time_stop1 >= houradmin) {
      ...
      NodeTransport nt{option.trip_id,
...
Routing::dijkstra_dalekowzrocznosc(
        ...
        if (result.empty()) {
          ...
          if (result.empty()) {
            ...
            std::chrono::zoned_time<std::chrono::seconds> arrival_at_B{
                time.get_time_zone(),
                record_of_distances[old_node].real_time.get_local_time() +
                    std::chrono::minutes(static_cast<int>(time_to_arrive))};
            NodeTransport nt{line_id, record_of_distances[old_node].real_time,
                             arrival_at_B};
            result.push_back(nt);
          }
        }
  ...
  // while (help[help.size()-1].stop_name != start)
  // help.push_back(prev[help[help.size()-1]]);
  ...
  for (const auto &node : help) {
    ...
    // TO JEST DO ZMIANY W ZALEZNOSCI OD TEGO JAKIE INFO CHCEMY! i chyba pasuje
    // jednak to dego NodeTimeInfo dodac nazwe liniji bo innaczje bedzie
    // pokazywac wczesniejsza i belive
    ...
    for (const auto &[new_stop, distance] :
      ...
      if (new_stop != old_node.stop_name) {
        ...
        std::vector<NodeTransport> result = transport_between_stops_reverse(
            new_stop, old_node.stop_name,
            record_of_distances[old_node].real_time);
        if (result.empty()) {
          /// change here with walking_between_stops_reverse
          result = walking_between_stops_reverse(
              new_stop, old_node.stop_name,
              record_of_distances[old_node].real_time);
          if (result.empty()) {
            // safty net - zeby graf sie nie rozspujnil
            int dist_meters = distance * 1609.344;
            time_to_arrive = std::ceil(dist_meters / this->walking_pace) *
                             5; // huge punishemnd so the algo is scared of this
                                // option, counted in minutes
            std::string line_id = "walk - safety net";
            /// minus over here:
            std::chrono::zoned_time<std::chrono::seconds> arrival_at_B{
                time.get_time_zone(),
                record_of_distances[old_node].real_time.get_local_time() -
                    std::chrono::minutes(time_to_arrive)};
            NodeTransport nt{line_id, record_of_distances[old_node].real_time,
                             arrival_at_B};
            result.push_back(nt);
          }
        ...
        for (const auto &node : result) {
          ...
          // this will be usefull if we will want to punish for walking:
          if (node.line_id == "walk")
            time_to_arrive *= walking_multiplier;
          if (old_node.line_id != "walk" and
              old_node.line_id != "walk - safety net")
            time_to_arrive += this->time_for_change;
          ...
          if (!record_of_distances.contains(n)) {
            ...
            if (record_of_distances[n].cost_from_start >
                old_cost + time_to_arrive) {
              NodeTimeInfo nt{old_cost + time_to_arrive, node.arrival};
              record_of_distances[n] = nt;
              pq.emplace(old_cost + time_to_arrive, n);
              prev[n] = old_node;
...
Routing::output(const std::string &start, const std::string &target,
                std::chrono::zoned_time<std::chrono::seconds> time) {
  std::vector<std::pair<Routing::Node, Routing::NodeTimeInfo>> arrival =
      dijkstra_dalekowzrocznosc(start, target, time);
  return arrival;
  /*//now how to we get an arrival time from dijsktra's arrival vector?
  std::chrono::zoned_time<std::chrono::seconds> arr_time =
  arrival[0].second.real_time; //= arrival[cos tam cos tam]?
  std::vector<std::pair<Routing::Node, Routing::NodeTimeInfo>> answer =
  dijkstra_dalekowzrocznosc(target, start, arr_time);
